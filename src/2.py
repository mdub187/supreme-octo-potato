...
columns_needed = ['number','title', 'ingredients', 'directions', 'link', 'source', 'NER', 'site']  # Adjust to your actual columns
    # Process each chunk
...
tree = ttk.Treeview(root, columns=['number', 'title', 'ingredients', 'directions', 'link', 'source', 'NER'], show='headings')
tree.pack()
        ...
        def show_recipe_details(recipe):
            ...
            def event3():
                ...
                def event3(source_button):
                    if event3:
                        on_tree_select_for_details=tree=Treeview.GetItem(Path).Rectangle
                        selected_item = tree.selection()[0]
